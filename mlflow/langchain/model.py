...
class _LangChainModelWrapper:
    ...
    def _update_dependencies_schemas_in_prediction_context(
        ...
        if (
            ...
            if not context:
                logger.warning(
                    "context is None in _update_dependencies_schemas_in_prediction_context"
                )
            else:
                logger.warning(
                    f"prediction context in _update_dependencies_schemas_in_prediction_context: {asdict(context)}"
                )
            if schema := _get_dependencies_schema_from_model(model) and context:
